...
def get_latents(chatbot_voice, xtts_model, voice_cleanup=False):
    ...
    if chatbot_voice not in latent_map:    
        speaker_wav = f"D:\\coding\\LLM\\speech to speech bot\\voice-bot-llm\\examples\\{chatbot_voice}.wav"
        if (voice_cleanup):
            try:
        ...
        for sentence in sentence_list:
            ...
            if audio_stream is not None:
                if not return_as_byte:
                    ...
                    return (history , gr.update(value=audio_unique_filename, autoplay=True))
                else:
                    return (history , gr.update(value=wav_bytestream, autoplay=True))
    except RuntimeError as e:
        if "device-side assert" in str(e):
            # cannot do anything on cuda device side error, need tor estart
            print(
